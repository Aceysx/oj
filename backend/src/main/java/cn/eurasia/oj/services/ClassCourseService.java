...
public class ClassCourseService {
    ...
    public Paper getClassCoursePaperBy(Long classCourseId, Long paperId) throws BusinessException {
        Paper paper = paperRepository.findById(paperId).orElseThrow(() -> new BusinessException("不存在该试卷"));
        ClassCoursePaper classCoursePaper = classCoursePaperRepository.findByClassCourseIdAndPaperId(classCourseId, paperId)
            .orElseThrow(() -> new BusinessException("不存在该考试信息"));
        paper.update(classCoursePaper.getEndTime(), classCoursePaper.getTimeBox());
        return paper;
    }
}
